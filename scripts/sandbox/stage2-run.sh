...
# Node is a client of the intercepting proxy, so it must trust the throwaway
# CA that signs the proxy's per-host leaf certificates. The real CA bundle is
# only for the proxy's own outbound TLS connection.
exec bwrap \
  --unshare-pid \
